...
public class BoardController {
    ...
    public String boardlist(Model model, @PageableDefault(page = 0, size = 10, sort = "id",
        ...
        // 데이터를 담아 페이지로 보내기 위해 Model자료형을 인자로,
            // 검색할 때 (searchKeyword가 있을 때) 안할 때 구분해 if문 사용
        ...
        int startPage = Math.max(nowPage - 4, 1); // Math.max(a,b) -- a 와 b 중 큰 값을 반환 -->
        // 그냥 nowPage - 4만 하면 nowpage가 1인 경우 -3도 가능하기에 이를 방지하기 위함.
    ...
    public String boardview(Model model, Integer id) { // localhost:8080/board/view?id=1 이런 식(get방식)
